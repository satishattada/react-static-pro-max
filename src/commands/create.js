...
async function create(name) {
  try {
    ...
    if (!fs.existsSync(templatePath)) {
      ...
      const altTemplatePath = path.resolve(process.cwd(), "templates", String(template));
      console.log(chalk.yellow(`Trying alternative path: ${altTemplatePath}\n`));
    ...
    if (installNow) {
      ...
      console.log(chalk.yellow("Remember to install dependencies before running the project:\n"));
      console.log(chalk.white(`  cd ${projectName}`));
      console.log(chalk.white(`  npm install\n`));
    }
  } catch (error) {
